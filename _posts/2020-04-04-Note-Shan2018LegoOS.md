...
* Modern Monolithic Server: Equipped with CPU, memory, and hard disk in each machine.
* Disadvantages of Monolithic Server:
    ...
    4. Poor support for the diversity of hardware. To introduce new hardware, this physical machine is usually replaced, because the new hardware is usually strongly bound to the motherboard of the physical machine.
* **Main Problem: How to improve resource utilization, elasticity, heterogeneity, and fault tolerance?**

## How

* New hardware makes breaking monolithic servers into network-attached, independent hardware components works (**Resource Disaggregation**).
    * Network: Infiniband (200Gbps,600ns); Optical Fabric (400Gbps,100ns)
    * Smart Devices: SmartNIC, SmartSSD, PIM
    * Network interface closer to the device: Omni-Path, Innova-2
* SplitKernel Architecture:
    * Split OS functions into monitors
    * Run each monitor at the hardware device
...
* Indirection layer to translate Linux ABI to the legoOS interface.
* Compute node contains CPU cache and ExCache (small RAM cache) to reduce network memory latency in any case.
* System structure:
...
* Various resources are essentially provided by different bare-metal machines, the monitor is implemented based on the Linux kernel, and there are still many performance problems in practice.